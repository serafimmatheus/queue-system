<x-layouts.auth-layout>
    <div class="main-card overflow-auto">
        ...
        <div class="flex gap-4 mb-4">
            <p class="bg-zinc-100 border-1 border-slate-300 rounded-md w-1/2 p-2">Estado: <span class="text-black font-bold">{!! getQueueStatusText($queue->status) !!}</span></p>
            <p class="bg-zinc-100 border-1 border-slate-300 rounded-md w-1/2 p-2">Criada em: <span class="text-black font-bold">{{ date('d/m/Y', strtotime($queue->created_at)) }}</span></p>
                ...
                <tbody>
                    @foreach ($tickets as $ticket)
                        <tr class="border-1 border-slate-300">
                            ...
                            <td class="border-1 border-slate-300">{{ date('d/m/Y', strtotime($ticket->queue_ticket_created_at)) }}</td>
                            <td class="border-1 border-slate-300">{!! getTicketStatus($ticket->queue_ticket_status) !!}</td>
                            <td class="border-1 border-slate-300">{{ $ticket->queue_ticket_called_at ? date('d/m/Y', strtotime($ticket->queue_ticket_called_at)) : '-' }}</td>
                            <td class="border-1 border-slate-300">{{ $ticket->queue_ticket_call_by ? $ticket->queue_ticket_call_by : '-' }}</td>
                        </tr>
                    @endforeach
                </tbody>
            </table>
